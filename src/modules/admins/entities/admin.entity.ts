 ...
 * `users`: admins inherit permissions from reusable roles, with legacy
 * direct grants retained only for compatibility. Global by nature — an admin is never scoped to a
 * tenant, so there is no `tenantId` here at all.
 */
@Entity('admins')
@ForeignKey('admin_roles', ['roleId'], ['id'], { name: 'admins_roleId_fkey', onDelete: 'RESTRICT' })
export class Admin {
